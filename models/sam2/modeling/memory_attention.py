...
class MemoryAttentionLayer(nn.Module):
    ...
    def _forward_sa(self, tgt, query_pos):
        # Self-Attention
        tgt2 = self.norm1(tgt)
        # Only add positional encodings when available
        if self.pos_enc_at_attn and (query_pos is not None):
            q = k = tgt2 + query_pos
        else:
            q = k = tgt2
        attn_out = self.self_attn(q, k, v=tgt2)
        # Some attention implementations may (incorrectly) return None in edge cases.
        # Fall back to a zero residual to avoid crashing.
        if attn_out is None:
            attn_out = torch.zeros_like(tgt2)
    ...
    def _forward_ca(self, tgt, memory, query_pos, pos, num_k_exclude_rope=0, rope_k_repeat=-1):
        ...
        # Cross-Attention
        tgt2 = self.norm2(tgt)
        q_in = tgt2 + query_pos if (self.pos_enc_at_cross_attn_queries and query_pos is not None) else tgt2
        k_in = memory + pos if (self.pos_enc_at_cross_attn_keys and pos is not None) else memory
        ca_out = self.cross_attn_image(
            q=q_in,
            k=k_in,
            v=memory,
            **kwds,

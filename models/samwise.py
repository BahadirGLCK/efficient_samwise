...
class SAMWISE(nn.Module):
    def __init__(self,
        ...
        # Some backbones (e.g., Hiera) expose `stage_ends`; generic TIMM backbones do not.
    ...
    def _early_fusion_stage(self, T, samples, txt, attention_mask):
        # Generic path for non-Hiera backbones (e.g., TIMM/RepViT): no trunk.blocks, fallback to features_only
        trunk = self.sam.image_encoder.trunk
        has_hiera_blocks = hasattr(trunk, 'patch_embed') and hasattr(trunk, 'blocks')
        if has_hiera_blocks:
            vis = trunk.patch_embed(samples)
            vis = vis + trunk._get_pos_embed(vis.shape[1:3])
            vis_outs = []
            ...
            for i, (i_v, i_t) in enumerate(zip(fusion_vis[:-1], fusion_txt[:-1])):
                vis = self.forw_layer_list(i_v, fusion_vis[i+1], trunk.blocks, vis)
    ...
    def _prepare_memory_conditioned_features(
        ...
        pix_feat_with_mem = self.sam.memory_attention(
            ...
            num_spatial_mem=num_spatial_mem_frames,
        )
...
def build_samwise(args):
    ...
    # Resolve SAM2 config and weights (allow overrides)
    use_preset = not bool(args.sam2_config)
    if use_preset:
        sam2_weights, sam2_config = SAM2_PATHS_CONFIG[args.sam2_version]
    else:
        sam2_config = args.sam2_config
        # In custom config mode, do not assume Hiera weights; allow checkpoint opt-in.
        sam2_weights = ''
    if args.sam2_checkpoint:
        sam2_weights = args.sam2_checkpoint
    if use_preset and not args.sam2_checkpoint:
    ...
    # Load weights if provided; skip for custom configs without a checkpoint (e.g., TIMM/RepViT)
    if sam2_weights and os.path.isfile(sam2_weights):
